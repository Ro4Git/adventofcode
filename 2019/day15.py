...
def test_program(stdscr):
    ...
    # 0 : not visited
    # 1 : wall 
    # 2,4,8,16 : mask de visite des voisins (deja visite)
    # 32 oxygen
    chars = (' ','█',
            '^','x',
            'v','x',
    ...
    while res != -1 and nbIteration < 3000:
        ...
        if (res == 0):
            wallpos = add(position,dirs[direction])
            paint(wallpos, 1)
            # find new direction to try ( all directions not tested yet)
            val = getColor(position)
            # allways right
            # direction = nextDirRight[direction]
            col = getColor(add(position,dirs[nextDirRight[direction]]))
            if col != 1 and col!=30:
                direction = nextDirRight[direction]
            else:
                for i in range(1,5):
            ...
            currentDist = getDist(position)
            position = add(position,dirs[direction])
            existingDist = getDist(position)
            if existingDist==0 or existingDist > currentDist:
                setDist(position, currentDist + 1)
            col = getColor(add(position,dirs[nextDirLeft[direction]]))
            if col != 1 and col!=30:
                direction = nextDirLeft[direction]
        elif res == 2:
            currentDist = getDist(position)
            position = add(position,dirs[direction])
            existingDist = getDist(position)
