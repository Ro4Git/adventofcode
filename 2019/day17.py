...
def printView():
    ...
    val = 0
    robotPos = (0,0)
    for i,strline in enumerate(strgrid):
        line = list(strline)
        for j,c in enumerate(line):
            if c == '#' and i>0 and j>0 and i<height-1 and j<width-1:
                if line[j-1] =='#' and line[j+1]=='#' and strgrid[i-1][j] == '#' and strgrid[i+1][j] == '#':
                    line[j] = 'O'
                    val = val + i*j
            if c != '.' and c!= '#':
                robotPos = (j,i)
    ...
    def validcoords(pos):
        x,y = pos
        return x>=0 and y>=0 and y<height and x<len(grid[y])
    ...
    dist = 0
    while res != -1:
        # keep direction until possible
        nextPos = add(pos, dirs[currentDir])


        if validcoords(nextPos) and (grid[nextPos[1]][nextPos[0]] != '.'):
            #keep going
            grid[nextPos[1]][nextPos[0]] = 'A'
            pos = nextPos
            dist += 1
        else:
            # register previous move 
            commandStr += ",{0}".format(dist)
            dist = 0
            #try right, left
            res = -1
            for t,turn in enumerate(turns):
                ...
                if validcoords(nextPos) and (grid[nextPos[1]][nextPos[0]] != '.'):
                    currentDir = dir
                    commandStr += ',L' if t==0 else ',R'
                    res = 0
                    break
    
    print(commandStr)
            
    inputsStr = "A,B,A,C,B,C,B,A,C,B" + chr(10) + "L,6,R,8,R,12,L,6,L,8" + chr(10) + "L,10,L,8,R,12" + chr(10) + "L,8,L,10,L,6,L,6" 
    Prog.reset()
    Prog.setMem(0,2)
