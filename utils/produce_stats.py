...
# and hand tuned
# generate a aoc%year%_stats.html page based on the current leaderbord json ("leaderboard_%year%.json") 
# downloaded from aoc
import json
...
def produceLeaderboardStats(aoc_year, aoc_leaderboard, sessionCookie):
    # download leaderboard information 
    download_aoc_file('https://adventofcode.com/' + aoc_year + '/leaderboard/private/view/' + aoc_leaderboard + ".json", "leaderboard_" + aoc_year + ".json",sessionCookie)
...
def download_aoc_file(url, local_filename, sessionCookie):
    cookieRo = {"session":sessionCookie}
...
def generate_fastest_solvers(members, days=25):
    ...
    for day in range(1, days + 1):
        ...
        if times:
            ...
            median_times[day] = int(median_time)
        else:
            median_times[day] = 0
...
def generate_html(hourly_data, top_part1, top_part2, fastest_part2, median_times, year):
    ...
    max_median_time = max([time for time in median_times.values() if time is not None] or [1])
    template_content = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            .median-chart th, .median-chart td {
    ...
    <body>
        ...
        <section class="median-chart">
            <h2>Median time to solve Part 2</h2>
            <table>
                <tr>
                ...
                <tr>
                    {% for day in range(1, 26) %}
                    <td>
                        {% if median_times[day] is not none %}
                        <div class="bar">
                            <div style="height: {{ median_times[day] * 100 // max_median_time }}%; background-color: rgb(255, {{ 255 - (median_times[day] * 255 // max_median_time) }}, 0);"></div>
                            <span>{{ median_times[day] // 60 }}m {{ median_times[day] % 60 }}s</span>
                        </div>
                        {% else %}
                        <span>--</span>
                        {% endif %}
        ...
        <section id="section-day-{{ day }}" class="section">
            ...
            <div class="three-columns">
                ...
                <div class="column">
                    <h3>Fastest Part1 -> Part2 (Median: {{ median_times[day] // 60 }}m {{ median_times[day] % 60 }}s)</h3>
                    <table>
                        <tr>
                            <th>Rank</th>
                            <th>Name</th>
    ...
    with open("aoc_" + year + "_stats.html", "w") as f:
